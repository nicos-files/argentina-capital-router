...
from src.recommendations.summary import is_empty_portfolio


def _constraints(plan: DailyCapitalPlan) -> Mapping[str, float]:
...
def build_daily_report_markdown(plan: DailyCapitalPlan) -> str:
    ...
    lines.append("")
    lines.append(
        f"- **manual_review_only:** {str(plan.manual_review_only).lower()}"
    )
    lines.append(
        f"- **live_trading_enabled:** {str(plan.live_trading_enabled).lower()}"
    )
    lines.append("- **no_orders:** true")
    lines.append("")

    is_empty = is_empty_portfolio(plan)
    if is_empty:
        lines.append("## Empty Portfolio")
        lines.append("")
        lines.append(
            "- Portfolio is currently empty (no positions, no cash balances)."
        )
        lines.append(
            "- Initial allocation is based on policy targets only."
        )
        lines.append(
            "- The portfolio snapshot used here is synthetic and was "
            "generated by the workflow."
        )
        lines.append("- Manual review is required. **No orders were placed.**")
        lines.append("")
    ...
    if constraints:
        ...
        lines.append("")
        min_trade = float(constraints.get("min_trade_usd", 0.0) or 0.0)
        max_alloc = int(
            constraints.get("max_allocations_per_contribution", 0) or 0
        )
        lines.append(f"- **Minimum trade size (USD):** {min_trade:.2f}")
        lines.append(
            f"- **Max allocations per contribution:** {max_alloc}"
        )
        lines.append("")
    ...
    lines.append("")
    lines.append(
        "> **These are not orders.** Review every line below and place each "
        "trade manually on your own broker, on your own time."
    )
    lines.append("")
    if plan.long_term_allocations:
        for alloc in plan.long_term_allocations:
            ...
            usd = float(alloc.get("allocation_usd", 0.0))
            lines.append(
                f"- [ ] **{symbol}** - USD {usd:.2f} _(bucket: {bucket})_"
            )
    else:
        lines.append(
            "- [ ] No long-term allocations recommended for this run. "
            "Review the rationale above before taking any action."
        )
    lines.append("")
    lines.append(
        "_Reminder: this tool never connects to a broker, never places "
        "orders, and never writes `execution.plan` or `final_decision.json`._"
    )
    lines.append("")
